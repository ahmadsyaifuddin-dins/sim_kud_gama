...
trait MemberReportsTrait
{
    public function generateMemberReport(Request $request, $reportType)
    {
        ...
        $view = '';
        $paper = 'portrait';
        $extraData = []; // Untuk menangani variabel spesifik (seperti $dataLahan atau status cetak)
        ...
        elseif ($reportType == 'kta') {
            ...
            // Tambahan khusus KTA: Menghitung jumlah yang sudah dan belum dicetak berdasarkan data yang difilter
            $extraData['sudahCetak'] = $data->where('status_cetak', 1)->count();
            $extraData['belumCetak'] = $data->where('status_cetak', 0)->count();
        }
