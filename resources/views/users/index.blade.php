                        ...
                        <td class="px-5 py-4 text-sm bg-white border-b border-gray-200 text-center">
                            <div class="flex justify-center space-x-3">
                                ...
                                <form action="{{ route('users.destroy', $user->id) }}" method="POST"
                                    class="inline-block confirm-action {{ Auth::id() == $user->id ? 'hidden' : '' }}"
                                    data-swal-title="Hapus User Ini?"
                                    data-swal-text="Apakah Anda yakin ingin menghapus akun {{ $user->name }}? Data tidak dapat dikembalikan.">
                                    @csrf
                                    @method('DELETE')

                                    <button type="submit"
                                        class="text-red-600 hover:text-red-900 transition p-1 rounded-md hover:bg-red-50"
                                        title="Hapus User">
